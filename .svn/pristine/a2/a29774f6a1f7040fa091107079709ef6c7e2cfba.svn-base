                   ...
                   success(function(data, status, headers, config) {
                        if(status == 200){
                           /* $localStorage.email = null;
                            ...
                            toaster.pop('error', "", "Unable to signout");
...
myApp.controller('BlurbsListCtrl', ['$scope', '$http', '$location', '$rootScope', '$routeParams', 'toaster', function($scope, $http, $location, $rootScope, $routeParams, toaster) {
	   ...
	   success(function(data, status, headers, config) {
        if(data.response){
    	   $scope.blurb_list = data.response;
            ...
            toaster.pop('info', "", 'No Blurbs found');
